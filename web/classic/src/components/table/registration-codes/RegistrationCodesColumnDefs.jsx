...
export const isExpired = (record) => {
  return (
    record.status === REGISTRATION_CODE_STATUS.ACTIVE &&
...
const renderStatus = (status, record, t) => {
  ...
  const statusConfig = REGISTRATION_CODE_STATUS_MAP[status];
  if (statusConfig) {
    return (
      ...
      render: (text, record) => {
        ...
        if (record.status === REGISTRATION_CODE_STATUS.ACTIVE && !isExpired(record)) {
          moreMenuItems.push({
            ...
            type: 'warning',
            onClick: () => {
              manageRegistrationCode(record.id, REGISTRATION_CODE_ACTIONS.DISABLE, record);
            },
          });
          ...
          moreMenuItems.push({
            ...
            type: 'secondary',
            onClick: () => {
              manageRegistrationCode(record.id, REGISTRATION_CODE_ACTIONS.ENABLE, record);
            },
            disabled: record.status === REGISTRATION_CODE_STATUS.USED,
          });
        }
        ...
        return (
          <Space>
            ...
            <Button
              ...
              disabled={record.status !== REGISTRATION_CODE_STATUS.ACTIVE}
            >
              {t('编辑')}
            </Button>
            <Dropdown
